...
def perform_cmeans_clustering(train_set: np.array, test_set: np.array,
                              no_clusters=3, m=2):
    center, train_labels = cmeans(train_set.T, no_clusters, m, error=0.005,
                                  maxiter=1000, init=None)[0:2]
    test_labels = cmeans_predict(test_set.T, center, m, 0.005, 1000)[0]
...
def main():
    ...
    train_clusters, test_clusters = perform_cmeans_clustering(train_data,
                                                              test_data)
